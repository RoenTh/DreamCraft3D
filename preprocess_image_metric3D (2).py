...
class Metric3D():
    def __init__(self, device='cuda'):
        self.device = device
        self.model = torch.hub.load('yvanyin/metric3d', 'metric3d_vit_large', pretrain=True).to(device)
    ...
    def __call__(self, image):
        image = Image.fromarray(image)
        rgb = self.transform(image).unsqueeze(0).to(self.device)
        pred_depth, confidence, output_dict = self.model.inference({'input': rgb})
        print(pred_depth.shape)
        pred_depth = F.interpolate(pred_depth, size=image.size[::-1], mode='bicubic', align_corners=False)
        return pred_depth.cpu().numpy(), confidence, output_dict
...
def preprocess_single_image(img_path, args):
    ...
    metric3d_model = Metric3D()  
    depth, confidence, output_dict = metric3d_model(image)
    print(depth.shape)
    depth = depth.squeeze()
    depth_resized = cv2.resize(depth, (mask.shape[1], mask.shape[0]), interpolation=cv2.INTER_LINEAR)
    depth_resized[mask] = (depth_resized[mask] - depth_resized[mask].min()) / (depth_resized[mask].max() - depth_resized[mask].min() + 1e-9)
    depth_resized[~mask] = 0
    depth = (depth_resized * 255).astype(np.uint8)


    pred_normal = output_dict['prediction_normal'][:, :3, :, :]
    #pred_normal = pred_normal.transpose(1, 2, 0)
    pred_normal = (pred_normal+1) / 2  # Values will be in range [0, 1]
    pred_normal = pred_normal.squeeze()
    pred_normal = F.interpolate(pred_normal.unsqueeze(0), size=(mask.shape[0], mask.shape[1]), mode='bicubic', align_corners=False).squeeze()
    normal = pred_normal.permute(1, 2, 0).cpu().numpy()
